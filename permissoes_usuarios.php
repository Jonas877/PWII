...
<?php 
    ...
    if ($resultado->num_rows > 0) {
        while ($row = $resultado->fetch_assoc()) {
                ...
                if($row["acao"] == 1)
                {
                    echo "<a href='permissoes_usuarios.php?id_usuario=$_GET[id_usuario]&excluir=$row[Id]' class='btn btn-danger'>Excluir Permissão</a>"; 
                }else{
                    echo "<a href='permissoes_usuarios.php?id_usuario=$_GET[id_usuario]&Adicionar=$row[permissao_id]' class='btn btn-success'> Adicionar Permissão</a>";
                }
                echo "</td>";
